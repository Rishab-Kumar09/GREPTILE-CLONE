...
export default function EnterpriseAnalysisPage() {
  ...
  const sendChatMessage = async (message: string) => {
    ...
    try {
      ...
      // 🔄 FALLBACK: Use GitHub API if RAG chat fails
      if (!response.ok) {
        console.log(`🔄 RAG chat failed (${response.status}), falling back to GitHub API...`)
      ...
      if (response.ok) {
        ...
        const chatSource = data.source || 'github-api'
